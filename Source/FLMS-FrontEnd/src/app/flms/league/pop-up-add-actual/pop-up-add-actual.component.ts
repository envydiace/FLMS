...
export class PopUpAddActualComponent implements OnInit {
  ...
  addActualFeetoList() {
    ...
    if (this.expenseName.trim() == null ||
      this.expenseName.trim().length > 50 ||this.expenseName.trim().length == 0||
      !pattern.test(this.expenseName.trim()) &&
      this.feeCost == null && this.type == null) {
      if (this.expenseName == null
        || this.expenseName.length > 50
        || !pattern.test(this.expenseName.trim())
      ) {
        this.isErrorEn = true;
      } else {
        if (this.expenseName.length > 50) {
          this.isErrorEn = true;
        }
        else if (this.expenseName.trim() == null) {
          this.isErrorEn = true;
