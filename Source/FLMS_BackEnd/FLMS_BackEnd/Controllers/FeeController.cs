...
namespace FLMS_BackEnd.Controllers
{
    ...
    public class FeeController : BaseApiController
    {
        ...
        [HttpPost("[action]")]
        [Authorize(Roles = "LEAGUE_MANAGER")]
        public async Task<ActionResult<AddLeagueFeeResponse>> AddLeagueFee(AddLeagueFeeRequest request)
        {
            var response = await feeService.AddListLeagueFee(request, UserID);
            if (response.Success)
            {
                return Ok(response);
            }
        ...
        [HttpPost("[action]/{LeagueFeeId}")]
        [Authorize(Roles = "LEAGUE_MANAGER")]
        public async Task<ActionResult<DeleteLeagueFeeResponse>> DeleteLeagueFee(int LeagueFeeId)
        {
            var response = await feeService.DeleteLeagueFee(LeagueFeeId, UserID);
            if (response.Success)
            {
                return Ok(response);
            }
